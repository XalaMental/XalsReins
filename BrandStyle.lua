...
Brand.TITLE_FONT_PATH = "Interface\\AddOns\\XalsReins\\Fonts\\CustomFont.ttf"
Brand.BODY_FONT_PATH = "Interface\\AddOns\\XalsReins\\Fonts\\FiraSans-Medium.ttf"
...
function Brand.ApplyBackgroundImage(f)
    ...
    img:SetAllPoints(f)
    img:SetTexture("Interface\\AddOns\\XalsReins\\Textures\\PanelBackground.jpg")
    return img
end
...
StaticPopupDialogs["XALSREINS_COPY_URL"] = {
    text = "%s",
    button1 = "Close",
    hasEditBox = true,
    editBoxWidth = 260,
...
function Brand.ShowCopyBox(caption, url)
    ...
    if not copyFrame then
        local f = CreateFrame("Frame", "XalsReinsCopyBox", UIParent)
        f:SetSize(360, 130)
